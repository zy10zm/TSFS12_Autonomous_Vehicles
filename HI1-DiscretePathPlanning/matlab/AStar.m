function res = AStar(num_nodes, mission, f, heuristic, num_controls)
    if nargin < 5
        num_controls = 0;
    end
    ...
    tic;
    q = PriorityQueue();
    q.insert(0 + heuristic(mission.start.id,mission.goal.id), mission.start.id);
    ...
    while ~q.isempty()
        ...
        for k=1:numel(neighbours)
            ...
            if previous(xi) == 0 || cost_to_come(xi) > cost_to_come(x) + di
                ...
                q.insert(cost_to_come(xi)+heuristic(xi,mission.goal.id),xi);
                if num_controls > 0
